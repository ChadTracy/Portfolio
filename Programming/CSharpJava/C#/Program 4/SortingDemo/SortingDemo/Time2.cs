...
public class Time2 : IComparable<Time2>
{
    ...
    public int CompareTo(Time2 t2)
    {
        ...
        int hourCompare = this.Hour - t2.Hour; // Hour difference?

        if (hourCompare != 0) // Check for hour difference first
            return hourCompare;

        int minuteCompare = this.Minute - t2.Minute; // Minute difference?

        if (minuteCompare != 0) // Check for minute difference next
            return minuteCompare;

        int secondCompare = this.Second - t2.Second; // Second difference?
